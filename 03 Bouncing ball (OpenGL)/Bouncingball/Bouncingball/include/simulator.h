...
bool ifCollision(glm::vec2 pos, glm::vec2 pos_before, Box box, int& idx, float& f)
{
    ...
    {
        if (glm::dot(min_to_posb, box.normal[0])==0)
        {
            return false;
        }
        if (glm::dot(min_to_pos, box.normal[0])==0)
        {
            idx_list.push_back(1);
            f_list.push_back(1.0f);
        }
        else
        {
            idx_list.push_back(1);
    ...
    {
        if (glm::dot(min_to_posb, box.normal[1])==0)
        {
            return false;
        }
        if (glm::dot(min_to_pos, box.normal[1]) == 0)
        {
    ...
    {
        if (glm::dot(max_to_posb, box.normal[2]) == 0)
        {
            return false;
        }
        if (glm::dot(max_to_pos, box.normal[2])==0)
        {
    ...
    {
        if (glm::dot(max_to_posb, box.normal[3])==0)
        {
            return false;
        }
        if (glm::dot(max_to_pos, box.normal[3])==0)
        {
    ...
    if (!idx_list.empty())
    {
        if (idx_list.size() == 1)
        {
            ...
            idx = idx_list[0];
            f = f_list[0];
            std::cout << "single collision and the f is " << f << std::endl;
            return true;
        }
...
void simulation(Ball& ball, Box box, float d_air, float d_wall, float bounce)
{
    ...
    while (TimeStepRemaining > EPSILON)
    {
        int idx = 0;
        // gravity
        ball.m_f = { 0.0f, -gravity * ball.m_mass };
        //wind
        ...
        if (ifCollision(pos, ball.m_pos, box, idx, f))
        {
            ...
            std::cout << "the speed is " << speed.x << "," << speed.y << std::endl;
            speed = CollisionResponse(speed, box, idx, bounce, d_wall);
            std::cout << "the after speed is " << speed.x << "," << speed.y << std::endl;
        }
